...
public class UserServiceImpl implements UserService {
    ...
    public Result register(User user) {
        ...
        // 检查用户是否已经存在
        User existingUserName = userMapper.selectOne(new QueryWrapper<User>().eq("user_name", user.getUserName()));
        if (existingUserName != null) {
            return new Result(401, "用户名已存在");
        }

        User existingNickName = userMapper.selectOne(new QueryWrapper<User>().eq("nick_name", user.getNickName()));
        if (existingNickName != null) {
            return new Result(401, "用户昵称已存在");
        }
        // 对用户密码进行加密
